...
def git_tools(workspace: str) -> list:
    ...
    def git_log(path: Optional[str] = None, max_count: int = 20) -> dict[str, Any]:
        ...
        n = min(n, 200)
        cmd = ["git", "-C", root, "log", f"-n{n}", f"--pretty=format:%h{_SEP}%an{_SEP}%ad{_SEP}%s", "--date=short"]
        if path:
            cmd += ["--", path]
        try:
    ...
    git_log.__aisuite_tool_metadata__ = ai.ToolMetadata(
        name="git_log", category="git", risk_level="low", capabilities=["git"], requires_approval=False
    )
