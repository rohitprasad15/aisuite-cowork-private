...
def test_text_attachment_is_inlined():
    content = build_user_content("", [{"kind": "text", "name": "notes.md", "text": "# Title\nbody"}])
    assert isinstance(content, list)
    assert content[0]["type"] == "text" and "notes.md" in content[0]["text"] and "# Title" in content[0]["text"]


def test_invalid_and_oversized_attachments_are_skipped():
    bad = [
        {"kind": "image", "data_url": "https://example.com/x.png"},  # not a data: URL
        {"kind": "image", "data_url": "data:image/png;base64," + "A" * 12_000_001},  # too big
...
def test_list_content_message_persists_and_titles(tmp_path):
    ...
    msgs = [{"role": "user", "content": build_user_content("review this diagram", [{"kind": "image", "data_url": url}])}]
    store = ConversationStore(tmp_path)
    rec = SessionRecord(session_id="s1", workspace=str(tmp_path), model="gpt-4o", mode="interactive", messages=msgs, agent="chat")
    store.save(rec)
